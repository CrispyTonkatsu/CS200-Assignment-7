...
const glm::vec4 O = cs200::point(0, 0), EX = cs200::vector(1, 0),
                EY = cs200::vector(0, 1);
...
private:
  ...
  GLuint program, texture_buffer, vbo[3], vao;
  GLint uobject_to_world, uworld_to_ndc, ulight_position, ulight_radius,
      ulight_factor, uambient_factor;
};
...
Client::Client(SDL_Window *w) : window(w) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    if (status != GL_TRUE) {
      string msg =
          "*** shader '" + string(files[i]) + "' failed to compile: ***\n";
      char buffer[1024];
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      sizeof(glm::vec4) * mesh.vertexCount(),
      mesh.vertexArray(),
      GL_STATIC_DRAW);
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      sizeof(glm::vec2) * mesh.vertexCount(),
      mesh.texcoordArray(),
      GL_STATIC_DRAW);
...
void Client::resize(int W, int H) {
  ...
  glm::mat4 world_to_ndc =
      cs200::cameraToNDC(camera) * cs200::worldToCamera(camera);
  glUseProgram(program);
...
void Client::keypress(SDL_Keycode kc) {
  ...
  switch (kc) {
    ...
    case SDLK_SPACE: mode = (mode + 1) % 3; break;
  ...
  ss << "Spot Light Shader Test [" << LABEL[mode] << ": " << fixed
     << setprecision(3) << values[mode] << "]";
